...
#%% Data analysis Data0
    def data_analysis(data0,data1, data2, data3, data4):
        ...
        #data0.State.value_counts(normalize=True)
...
#%% Logistic Regression data0
    ...
    LogRegr0= LogisticRegression(max_iter = 1000)
    ...
    from sklearn.linear_model import LogisticRegression
    LogRegr1= LogisticRegression(max_iter = 1000)
    ...
    from sklearn.linear_model import LogisticRegression
    LogRegr2= LogisticRegression(max_iter = 1000)
    ...
    # Uncomment next section for using alternative method for MLR:
    # also indicates the p-value and intervals of the coefficients
    ...
    # for var in vars:
    #     formula += var + ' + '
    # print(sm.add_constant(data3[vars + ['Current_state',]]).info())
    # print(formula[:-3])
    # log_reg3 = sm.MNLogit(Y_train,sm.add_constant(X_train))#formula[:-3], data = sm.add_constant(data3[vars + ['Current_state',]]))#Y_train,sm.add_constant(X_train))
    # log_reg3_res = log_reg3.fit(method = 'lbfgs', maxiter = 10000)
    # print(log_reg3_res.summary())
    # print(log_reg3_res.summary2())
    # print(log_reg3_res.params)
    # LogRegr3 = log_reg3_res
    ...
    from sklearn.linear_model import LogisticRegression
    LogRegr4= LogisticRegression(max_iter = 1000)
...
def test_models(link,lane, test_folder, save_folder, downstream_folder, variables,xmin= None, xmax=None, p = 0.10, plot = 0):
    # Load models
    import pickle
    filename= save_folder + '\\LR_model_state0_downstream_' + str(int(link)) + str(int(lane)) + '.sav'
    ...
    data_sample = data_test.sample(frac = p) # sample size = p
    for i in range(len(cycles)):
    ...
    ## uncomment this for distribution of the probabilities
    # print(df)
    # a = df[['U', 'GroundTruth']]
    ...
    SE_sp = df['Sp'].to_list()
    SE_sp2 = [[1-SE_sp[i], SE_sp[i]] for i in range(len(SE_sp))]
    LL_sp = log_loss(GT_sp, SE_sp2)
...
#%% Plot outcome probabilities
    if plot == 1:
        ...
        plt.show()
    return accuracy, log_loss(list(y.values()), state_est[1:]), AUC, accuracy_Spillover, false_detection, LL_sp