...
#%% Train logistic regression model
    ...
    X_train = X[variables + ['Downstream_state',]]
    Y_train = X[['Current_state']]
    LogRegr = LogisticRegression(max_iter = 10000)
    LogRegr.fit(X_train, Y_train)

    import pickle
    filename = save_folder + '\\LR_model_state_downstream' + str(int(link)) + str(int(lane)) + '.sav'
    pickle.dump(LogRegr, open(filename, 'wb'))
    return
#%% Test data
def test_models(link,lane, test_folder, model_folder,downstream_folder, variables,xmin = 200, xmax = 300, p= 0.10, plot = 0):

    # Load models
    import pickle
    filename = model_folder + '\\LR_model_state_downstream' + str(int(link)) + str(int(lane)) + '.sav'
    ...
    data_sample = data_test.sample(frac=p)
    ...
    for i in range(len(cycles)):
        ...
        if not data_current.empty:
            ...
            x[cycles[i]] = {'Current_cycle': cycles[i], 'TT_on_link': TT,
                            'Average_stopping_time': avg_stopping, 'Min_stop': min_stop, 'Max_stop': max_stop, 'Number_of_stops': nb_stops,
                            'Time_on_intersection': time_on_intersection,'Shockwave_intersection': shockwave}
        else:
            if i == 0:
                x[cycles[0]] = {'Current_cycle': cycles[0], 'TT_on_link': 35, 'Average_stopping_time':0, 'Min_stop': 20,
                                'Max_stop': 0,
                                'Number_of_stops': 0,'Time_on_intersection': 4.2, 'Shockwave_intersection': 0.2}
            else:
...
#%% Validation
    ...
    X_test = X_test[variables + ['Downstream_state',]]
    Y_test= Y_test[['State']]
    ...
    AUC = roc_auc_score(GT, state_est[:])
...
#%% Plot outcome probabilities
    ...
    #return veh_sample
    return accuracy, logloss, AUC, accuracy_Spillover, false_detection, LL_sp