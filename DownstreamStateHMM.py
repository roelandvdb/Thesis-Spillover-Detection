...
def downstream_state(training_folder, test_folder, model_folder, downstream_file_training, downstream_file_test, variables):
    ...
    for link in [44.0]:
        for lane in [2.0,3.0]:
            ...
            import pickle
            filename = model_folder + '\\LR_model_state0_' + str(int(link)) + str(int(lane)) + '.sav'
            LogRegr0 = pickle.load(open(filename, 'rb'))
            filename = model_folder + '\\LR_model_state1_' + str(int(link)) + str(int(lane)) + '.sav'
            LogRegr1 = pickle.load(open(filename, 'rb'))
            filename = model_folder + '\\LR_model_state2_' + str(int(link)) + str(int(lane)) + '.sav'
            LogRegr2 = pickle.load(open(filename, 'rb'))
            filename = model_folder + '\\LR_model_state3_' + str(int(link)) + str(int(lane)) + '.sav'
            LogRegr3 = pickle.load(open(filename, 'rb'))
            filename = model_folder + '\\LR_model_state4_' + str(int(link)) + str(int(lane)) + '.sav'
            ...
            # Find missing links in data (to set transition prob. to 0)
            states = [0, 1, 2, 3, 4]
            ...
            # Get Transition matrix --> get_transition_matrix(TT, nb_stops, Average_stopping_time, Time_on_intersection, Shockwave, states, not_in_regr)
            ...
            for i in range(len(cycles)):
                obs = X.loc[X.index == cycles[i]].to_numpy()
