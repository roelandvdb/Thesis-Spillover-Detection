...
def train_models(link, lane, training_folder,save_folder, variables):
    ...
    # Data analysis: uncomment call at the bottom
...
def test_models(link, lane, test_folder, save_folder, variables, xmin=None, xmax=None, plot = 0):
    # load models
    import pickle
    filename= save_folder + '\\LR_model_state0_' + str(int(link)) + str(int(lane)) + '.sav'
    ...
    for state in states: # if the transition not in the training dataset --> set to 0
    ...
    for i in range(len(cycles)):
        ...
        TM = get_transition_matrix(obs[0][1:]) #get transition matrix with the observed cov.
    ...
    try:
        print(log_loss(list(y.values()), state_est[1:]))
    except:
